...
import { createOfficeHoursRecord, updateOfficeHoursRecord } from '../../lib/supabase';
...
interface OfficeHoursRecordFormProps {
  studentId: string;
  coachId: string;
  initialData?: OfficeHoursRecord | null;
  onSuccess: () => void;
  onCancel: () => void;
}
...
export function OfficeHoursRecordForm({
  ...
  const [record, setRecord] = useState<Omit<OfficeHoursRecord, 'id' | 'created_at' | 'updated_at'>>({
    ...
    session_date: initialData?.session_date || new Date().toISOString(),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const formattedRecord = {
        ...record,
        session_date: new Date(record.session_date).toISOString(),
      };

      if (initialData) {
        await updateOfficeHoursRecord(initialData.id, formattedRecord);
        addToast('Office hours record updated successfully', 'success');
      } else {
        await createOfficeHoursRecord(formattedRecord);
              ...
              <div>
                ...
                <div className="mt-4 grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input 
                      type="datetime-local"
                      value={typeof record.session_date === 'string' ? record.session_date.slice(0, 16) : new Date().toISOString().slice(0, 16)}
