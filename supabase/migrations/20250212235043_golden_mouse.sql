-- Disable Supabase Auth built-in email sending
ALTER TABLE auth.users DISABLE TRIGGER _email_trigger;
...
-- Update RPC function to ensure it triggers our email handler
...
BEGIN
  ...
  -- Ensure the email is queued by explicitly calling the handler
  PERFORM handle_password_reset_request();
END;
$$;